...
- Bun for installs/runs; npm works with `--legacy-peer-deps` because `cmdk@0.2.1` has a React 18 peer.
...
npm run db:seed                 # load Drive seeds (truncates files)
bun run dev                     # Next dev on port 4000
```
Open http://localhost:4000.

> If you need npm: `npm install --legacy-peer-deps` then use the same db/dev commands (npm scripts are defined).

## Drive data layer
- Commands: `npm run db:up` / `db:down` / `db:migrate` / `db:seed`.
...
- Run `npm run db:down` to stop the local Postgres container (volume persists unless pruned).
